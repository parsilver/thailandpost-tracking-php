...
it('should subscribe by barcodes success', function () {
    $expectJsonStringResponse = json_encode(['response' => 'success']);

    $storage = $this->createMock(StorageRepositoryInterface::class);
    $storage->method('has')->with('access-token:webhook')->willReturn(true);
    $storage->method('get')->with('access-token:webhook')->willReturn(
        json_encode([
            'token' => 'valid-token',
    ...
    $api = new WebhookEndpoint($client);

    $response = $api->subscribeByBarcodes([
